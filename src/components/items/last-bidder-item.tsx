...
export const LastBidders = ({ bidDate, name, amount, index }: IProps) => {
  ...
  return (
    <li className='flex-row flex gap-2 items-center  justify-between py-4'>
      <p
        id={`last-bid-time-ago-${index}`}
        className='text-[#666666] items-center w-full'
      >
        {getRelativeTime(new Date())}
      </p>
      <p className='font-medium truncate items-center w-full'>{name}</p>
      <p className='text-[#666666] w-full items-center'>
