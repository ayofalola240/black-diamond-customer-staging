  ...
  const columns: ColumnDef<Bid>[] = [
    {
      ...
      header: "Status",
      cell: (row) => {
        const realAuction = isUseInAllBids
          ? row.cell.row.original.auction
          : auction;
        ...
        return (
          <div className="flex gap-2 min-w-[300px] sm:min-w-auto flex-row">
            <div className="relative overflow-hidden rounded-[8px] border-[#EEEEEE] w-[160px] h-[160px]">
              <Image
                src={realAuction?.images[0]?.image_url}
                alt={realAuction?.title}
                fill
                className="object-contain"
              />
            </div>
            <div className="flex flex-col justify-center gap-2">
              <p className="font-medium line-clamp-1 text-ellipsis text-base sm:text-xl">
                {realAuction?.title}
              </p>
              <p className="text-[#666] text-sm">
                Starting Bid: ₦{realAuction.startingBidAmount.toLocaleString()}
              </p>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <p className="font-medium">₦{row.original.amount.toLocaleString()}</p>
        );
      },
    },
    {
