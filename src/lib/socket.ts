...
const attemptReconnect = () => {
  if (retryAttempts < maxRetries) {
    retryAttempts++;
    console.log(
      `Retrying to connect... Attempt ${retryAttempts} of ${maxRetries}`
    );
    setTimeout(() => {
    ...
    console.error(
      "Max retry attempts reached. Could not reconnect to the socket server."
    );
  }
};
...
export const getSocket = (): Socket => {
  if (!socket) {
    throw new Error(
      "Socket connection has not been initialized. Call initializeSocket first."
    );
  }
  return socket;
};
